...
void* init_test(long long data_size, cudaStream_t stream) {
    auto config = new StridedFFTConfig<FFTSize, FFTsInBlock, true, 1>();
    config->init(stream);
...
void run_test(void* plan, cufftComplex* d_data, cufftComplex* d_kernel, long long total_elems, cudaStream_t stream){
    static_cast<StridedFFTConfig<FFTSize, FFTsInBlock, true, 1>*>(plan)->execute_fft(d_data, total_elems, stream);
}

template<int FFTSize, int FFTsInBlock, int exec_mode>
void delete_test(void* plan) {
    delete static_cast<StridedFFTConfig<FFTSize, FFTsInBlock, true, 1>*>(plan);
}