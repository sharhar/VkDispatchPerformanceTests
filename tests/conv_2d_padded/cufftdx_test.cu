...
__global__ void zero_input(cufftComplex* data, long long total_elems) {
    long long i = blockIdx.x * 1LL * blockDim.x + threadIdx.x;
    if (i >= total_elems) {
        return;
    }

    long long x_coord = i % FFTSize;
    long long y_coord = (i / FFTSize) % FFTSize;
    //long long image_idx = i / (FFTSize * FFTSize);

    if (x_coord >= FFTSize / PADDING_RATIO || y_coord >= FFTSize / PADDING_RATIO) {
    //if (y_coord >= FFTSize / PADDING_RATIO) {
        data[i] = make_float2(0.0f, 0.0f);
    }
}
